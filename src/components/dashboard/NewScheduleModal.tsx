...
export default function NewScheduleModal({ isOpen, onClose, selectedDate, onScheduled, profile }: Props) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      fetch('/api/leads').then(r => r.json()).then((data: { leads: Lead[] }) => setAllLeads(data.leads ?? [])).catch(() => setAllLeads([])).finally(() => setLeadsLoading(false)),
      profile.role === 'manager'
        ? fetch('/api/team/members').then(r => r.json()).then((data: { members: RepMember[] }) => setReps(data.members ?? [])).catch(() => {})
        : Promise.resolve(),
    ])
  }, [isOpen, selectedDate, profile.id, profile.role])
  ...
  return (
    <div className="fixed inset-0 flex items-end justify-center md:items-center" style={{ zIndex: 1100 }}>
      ...
      <div
        ...
        style={{ background: 'var(--bg-card)', borderRadius: '24px 24px 0 0', padding: '24px', paddingBottom: 90, maxHeight: '90vh' }}
      >
        ...
        <div style={{ marginBottom: '20px' }}>
          <p style={labelStyle}>Select Lead *</p>
          {pickedLead ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(217,119,6,0.1)', borderRadius: '10px', padding: '12px 14px' }}>
              <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: STATUS_COLOR[pickedLead.status], flexShrink: 0 }} />
              <span style={{ flex: 1, fontSize: '15px', color: 'var(--label-primary)' }}>{pickedLead.cafe_name}</span>
              <button type="button" onClick={() => setPickedLead(null)} style={{ color: 'var(--label-tertiary)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                <X size={14} />
              </button>
            </div>
          ) : (
            <div>
              <div style={{ position: 'relative', marginBottom: '8px' }}>
                <Search size={14} style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)', color: 'var(--label-tertiary)', pointerEvents: 'none' }} />
                <input type="text" placeholder="Search cafes…" value={search} onChange={e => setSearch(e.target.value)} style={{ ...inputStyle, paddingLeft: '36px' }} className="placeholder:text-[rgba(235,235,245,0.3)]" />
              </div>
              <div style={{ maxHeight: '144px', overflowY: 'auto', borderRadius: '10px', background: 'rgba(118,118,128,0.08)' }}>
                {leadsLoading ? (
                  ...
                  filteredLeads.map(l => (
                    ...
                    >
                      <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: STATUS_COLOR[l.status], flexShrink: 0 }} />
                      <span style={{ flex: 1, fontSize: '14px', color: 'var(--label-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{l.cafe_name}</span>
                      <span style={{ flexShrink: 0, borderRadius: '6px', padding: '2px 6px', fontSize: '11px', fontWeight: 500, background: `${STATUS_COLOR[l.status]}20`, color: STATUS_COLOR[l.status] }}>{STATUS_LABEL[l.status]}</span>
                    </button>
                  ))
                )}
              </div>
        ...
        <button
          ...
          style={{ width: '100%', height: '50px', borderRadius: '14px', background: '#D97706', color: '#FFF', fontSize: '17px', fontWeight: 600, border: 'none', cursor: !pickedLead || !scheduledDate || submitting ? 'not-allowed' : 'pointer', opacity: !pickedLead || !scheduledDate || submitting ? 0.5 : 1, fontFamily: 'inherit' }}
        >
